...
io.on('connection', (socket) => {
    ...
    // 🔥 2. Chat Message Event (FIXED: msgType ko DB mein save karwaya) 🔥
    socket.on('chatMessage', async (data) => {
        // Frontend ab text ke sath msgType ('text' ya 'image') bhi bhejega
        const { room, senderName, text, msgType } = data; 
        try {
            const newMessage = await MessageModel.create({
                roomId: room,
                sender: senderName,
                content: text, // Agar image hai toh yahan ImageKit ka URL aayega
                msgType: msgType || 'text' // Database ko type pata chal jayega
            });
            io.to(room).emit('message', newMessage);
        } catch (err) {
            console.error("Error saving message:", err);
    ...
    // 4. Message Seen Event (Ticks ke liye)
    socket.on('markAsSeen', async ({ messageId, room }) => {
        try {
            await MessageModel.findByIdAndUpdate(messageId, { status: 'seen' });
            io.to(room).emit('messageStatusUpdated', { messageId, status: 'seen' });
